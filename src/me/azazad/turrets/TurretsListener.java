...
public class TurretsListener implements Listener {
			...
			if (pcs.getTurretCreationStep() == 1 && pcs.getLockedState()) {
				...
				if (event.getAction() == Action.LEFT_CLICK_BLOCK) {
					...
					if (clickedBlock.getType() == Material.CHEST) {
						...
						if (turret.checkIfBlockByTurret(clickedBlock)) {
							if (turret.addTurretAmmoBoxChest(clickedBlock))
								player.sendMessage("Ammo box linked to turret!");
							else
								player.sendMessage(ChatColor.RED + "Ammo box linkage failed! Already linked?");
				...
				if (event.getAction() == Action.LEFT_CLICK_BLOCK) {
					...
					if (clickedBlock.getType() == Material.CHEST) {
						Turret turret = plugin.getPlayerCommander(player).getTurretSelected();
						if (turret.checkIfBlockByTurret(clickedBlock)) {
							if (turret.getTurretAmmoBox().removeAmmoChest(clickedBlock))
								player.sendMessage("Ammo box link severed!");
							else
								player.sendMessage(ChatColor.RED + "Ammo box severing failed! Ammo box not linked to this turret?");
							plugin.playerCommanders.remove(pcs);
						}
					}
				}
				...
				if (player.hasPermission("turrets.addturret")) {
					...
					if (plugin.canBuildTurret(postLocation)) {
						...
						if (plugin.getTurretOwner(player.getName()) == null) {
							...
							turretOwner = plugin.getTurretOwner(player.getName());
						if ((turretOwner.getNumTurretsOwned() < turretOwner.getMaxTurretsAllowed()) || player.hasPermission("turrets.ignoremaxturrets")) {
							...
							player.sendMessage("You are already at your maximum number of turrets!");
					} else
						plugin.notifyPlayer(player, TurretsMessage.TURRET_CANNOT_BUILD);
				} else
					player.sendMessage(ChatColor.RED + "You don't have permissions to create a turret!");
			}
		}
	}
	...
	public void onPlayerLogin(PlayerLoginEvent event) {
		if (plugin.getTurretOwner(event.getPlayer()) != null)
			plugin.getTurretOwner(event.getPlayer()).refreshOnlineStatus();
	}

	@EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
	public void onPlayerJoin(PlayerJoinEvent event) {
		// int viewStartX = (event.getPlayer().getLocation().getChunk().getX() - Bukkit.getServer().getViewDistance())
		// << 4;
		// int viewStartZ = (event.getPlayer().getLocation().getChunk().getZ() - Bukkit.getServer().getViewDistance())
		// << 4;
		// int viewEndX = (event.getPlayer().getLocation().getChunk().getX() + Bukkit.getServer().getViewDistance()) <<
		// 4;
		// int viewEndZ = (event.getPlayer().getLocation().getChunk().getZ() + Bukkit.getServer().getViewDistance()) <<
		// 4;
		// BlockLocation bloc;
		...
		if (material == Material.CHEST) {
			for (Turret turret : plugin.getTurrets()) {
				if (turret.getTurretAmmoBox().isChestAttachedToTurret(block)) {
					if (block.getRelative(BlockFace.NORTH).getType().equals(Material.CHEST) || block.getRelative(BlockFace.EAST).getType().equals(Material.CHEST) || block.getRelative(BlockFace.SOUTH).getType().equals(Material.CHEST)
						...
						if (turret.getTurretAmmoBox().removeAmmoChest(block))
